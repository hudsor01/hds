...
export type Database = {
  public: {
    ...
    Enums: {
      ...
      PaymentStatus: 'PENDING' | 'PAID' | 'COMPLETED' | 'FAILED' | 'CANCELLED' | 'REFUNDED' | 'OVERDUE' | 'LATE' | 'PARTIAL'
      PaymentType: 'RENT' | 'DEPOSIT' | 'SUBSCRIPTION' | 'LATE_FEE' | 'MAINTENANCE' | 'UTILITIES' | 'SECURITY_DEPOSIT' | 'OTHER'
      PropertyStatus: 'AVAILABLE' | 'OCCUPIED' | 'VACANT' | 'MAINTENANCE' | 'RENOVATION' | 'OFF_MARKET'
      PropertyType: 'SINGLE_FAMILY' | 'MULTI_FAMILY' | 'APARTMENT' | 'CONDO' | 'TOWNHOUSE' | 'COMMERCIAL' | 'INDUSTRIAL' | 'LAND'
      RequestStatus: 'PENDING' | 'APPROVED' | 'REJECTED'
      SubscriptionStatus: 'ACTIVE' | 'PAST_DUE' | 'CANCELLED' | 'INCOMPLETE' | 'INCOMPLETE_EXPIRED' | 'TRIALING' | 'UNPAID'
      TenantStatus: 'ACTIVE' | 'INACTIVE' | 'PENDING' | 'REJECTED'
      ToolType: 'PROPERTY_MANAGER' | 'PROJECT_MANAGER' | 'IMAGE_OPTIMIZER' | 'DOCUMENT_MANAGER'
      UserRole: 'SUPER_ADMIN' | 'ADMIN' | 'PROPERTY_MANAGER' | 'PROPERTY_OWNER' | 'MAINTENANCE' | 'TENANT' | 'USER'
    }
    CompositeTypes: {
      [_ in never]: never
    }
