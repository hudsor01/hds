...
import { MoreVertical, Edit, Trash2, Users, FileText, Tool, DollarSign, Camera } from 'react-feather'
...
export function PropertyActions({
  ...
  return (
    <Box className={cn('flex items-center', className)}>
      <Tooltip title={disabled ? 'Actions not available' : 'Property Actions'} placement={tooltipPlacement}>
        <span>
          <IconButton
            ...
            disabled={disabled}
            size={size}
            className={cn('transition-colors', disabled ? 'text-gray-300' : 'text-gray-600 hover:text-gray-900')}
          >
      ...
      <Dialog open={Boolean(confirmAction)} onClose={handleCancelConfirmation} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button onClick={handleConfirmAction} color={confirmAction?.color || 'primary'} variant="contained" autoFocus>
            Confirm
          </Button>
        </DialogActions>
      </Dialog>
