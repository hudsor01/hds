...
import { type MaintenanceRequest } from '@/types/maintenance_requests'
...
export default function MaintenanceTracker({ propertyId }: { propertyId: string }) {
  ...
  const supabase = supabase(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!)
      ...
      .on(
        ...
        payload => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            setRequests(prev =>
              prev.map(request => (request.id === payload.new.id ? (payload.new as MaintenanceRequest) : request))
            )
            toast({
        ...
        <div className="space-y-4">
          {requests.map(request => (
            <div key={request.id} className="flex items-center justify-between rounded-lg border p-4">
              ...
              <Badge
                variant={request.status === 'completed' ? 'secondary' : request.status === 'in_progress' ? 'outline' : 'default'}
              >
                {request.status.replace('_', ' ')}
              </Badge>
            </div>
          ))}
          {requests.length === 0 && <p className="text-center text-gray-500">No maintenance requests found</p>}
        </div>
      </CardContent>
    </Card>
  )
