...
const StyledPopover = styled(MuiPopover)(({ theme }: { theme: Theme }) => ({
  '& .MuiPaper-root': {
...
export const PopoverTrigger = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(
  ({ className, ...props }, ref) => <button ref={ref} type="button" className={cn(className)} {...props} />
)
...
export const PopoverContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('bg-popover text-popover-foreground z-50 w-72 rounded-md p-4 shadow-md outline-hidden', className)}
      {...props}
    />
  )
)
