...
>(({ className, orientation = 'vertical', ...props }, ref) => {
  ...
  return (
    <FormItemContext.Provider value={{ id }}>
      <StyledFormItem
        ref={ref}
        className={cn(orientation === 'horizontal' && 'flex items-center gap-4', className)}
        {...props}
      />
...
>(({ className, required, optional, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      {optional && <span className='text-gray-500 ml-1 text-sm'>(optional)</span>}
    </InputLabel>
  );
});
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <FormHelperText
      ...
      className={cn('text-sm text-gray-500 transition-opacity', className)}
      {...props}
    />
  );
});
