...
export function SessionManager() {
  ...
  const revokeSession = async (sessionId: string) => {
    try {
      ...
      toast.error(`Failed to revoke session: ${error instanceof Error ? error.message : 'Unknown error'}`)
    } finally {
      setIsRevoking(null)
    }
  }
              ...
              {sessions.length > 1 && (
                ...
                >
                  <div className="text-xs text-muted-foreground">
                    Revoking a session will force a sign out on that device. The current session cannot be revoked.
                  </div>
                </motion.div>
              )}
            </motion.div>
