...
export function TwoFactorSetup() {
  ...
  const handle2FASetup = async () => {
    try {
      ...
      toast.error(`Failed to setup 2FA: ${error instanceof Error ? error.message : 'Unknown error'}`)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleVerify2FA = async () => {
    try {
      ...
      toast.error(`Invalid verification code: ${error instanceof Error ? error.message : 'Unknown error'}`)
    } finally {
      setIsLoading(false)
    }
  }
