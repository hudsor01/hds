...
export async function POST(req: NextRequest) {
    try {
        ...
        const { data: recentOrders, error: countError } =
            await supabase
                .from('work_orders')
        ...
        if (countError) {
            console.error(
                'Error checking work order count:',
                countError
            )
            return NextResponse.json(
                { error: 'Failed to validate work order limit' },
                { status: 500 }
            )
        }
...
export async function PUT(req: NextRequest) {
    try {
        ...
        if (existingWorkOrder.status === 'CANCELLED') {
            return NextResponse.json(
                { error: 'Cannot update cancelled work orders' },
                { status: 400 }
            )
        }

        const validatedData = workOrderSchema
            .partial()
            .parse(updateData)
        ...
        if (
            ...
            validatedData.vendor_id !== existingWorkOrder.vendor_id
        ) {
        ...
        if (
            ...
            validatedData.status !== existingWorkOrder.status
        ) {
            const { error: notificationError } = await supabase
                .from('notifications')
                .insert([
...
export async function DELETE(req: NextRequest) {
    try {
        ...
        const { data: workOrder, error: workOrderCheckError } =
            await supabase
                .from('work_orders')
        ...
        if (workOrder.status === 'IN_PROGRESS') {
            return NextResponse.json(
                { error: 'Cannot delete in-progress work orders' },
                { status: 400 }
            )
        }
