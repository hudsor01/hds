...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const { email, firstname }: { email: string; firstname: string } = await request.json()
  const { data, error }: { data: any; error: any } = await resend.emails.send({
    from: 'Richard Hudson<rhudsontspr@gmail.com>',
    to: [email],
    subject: 'Thank you for waitlisting the Hudson Digital Solutions - Property Management System!',
    replyTo: 'rhudsontspr@gmail.com',
    html: await render(WelcomeTemplate({ position: firstname, referralCode: '' }))
  })
