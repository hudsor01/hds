...
export async function GET(req: NextRequest): Promise<NextResponse> {
    try {
        const { userId }: { userId: string | null } =
            await supabase.auth()
        if (!userId) {
        ...
        const { data: leases, error }: { data: any; error: any } =
            await query
...
export async function POST(req: NextRequest): Promise<NextResponse> {
    try {
        ...
        const { data: lease, error }: { data: any; error: any } =
            await supabase
                .from('leases')
