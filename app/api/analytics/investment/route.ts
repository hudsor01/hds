...
export async function GET(request: Request): Promise<NextResponse> {
    try {
        ...
        if (propertyId) {
            ...
            const { data: roiMetrics, error: roiError } =
                (await supabase.rpc('calculate_property_roi', {
        ...
        if (organizationId) {
            ...
            const { data: portfolioMetrics, error: portfolioError } =
                (await supabase.rpc(
            ...
            const { data: performanceReport, error: reportError } =
                (await supabase.rpc(
