...
export async function GET(req: NextRequest) {
    try {
        ...
        const settingsType =
            req.nextUrl.searchParams.get('type') || 'user'
...
export async function PUT(req: NextRequest) {
    try {
        ...
        let validatedData
        if (settingsType === 'security') {
            validatedData = userSecuritySettingsSchema
                .partial()
                .parse(updateData)
        } else {
            validatedData = userSettingsSchema
                .partial()
                .parse(updateData)
        }
        ...
        const { data: settings, error } = await supabase
            ...
            .eq(
                settingsType === 'security' ? 'user_id' : 'id',
                userId
            )
            .select()
            .single()
            ...
            await supabase.from('notifications').insert([
                {
                    ...
                    data: {
                        ...
                        two_factor_enabled:
                            validatedData.two_factor_enabled
                    }
                }
            ])
        }
...
export async function DELETE(req: NextRequest) {
    try {
        ...
        const { data: existingSetting, error: getError } =
            await supabase
                .from('system_configurations')
                .select('*')
                .eq('key', key)
