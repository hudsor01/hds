...
export async function GET(
    ...
    try {
        ...
        const { data: expenses, error: expensesError } =
            await supabase.rpc('calculate_property_expenses', {
        ...
        const { data: metrics, error: metricsError } =
            await supabase.rpc('calculate_expense_metrics', {
                p_property_id: params.id,
                p_start_date: startDate,
                p_end_date: endDate
            })
        ...
        const { data: profitLoss, error: plError } =
            await supabase.rpc('calculate_profit_loss', {
                p_property_id: params.id,
        ...
        return NextResponse.json(
            { error: 'Error calculating property expenses' },
            { status: 500 }
        )
    }
}
