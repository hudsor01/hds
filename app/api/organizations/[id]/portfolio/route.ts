    ...
    try {
        ...
        const { data: membership, error: membershipError } =
            await supabase
                ...
                .eq(
                    'user_id',
                    (await supabase.auth.getCurrentUser()).data.user
                        ?.id
                )
                .single()
        ...
        const { data: metrics, error: metricsError } =
            await supabase.rpc('calculate_portfolio_metrics', {
        ...
        const { data: growth, error: growthError } =
            await supabase.rpc('calculate_yoy_growth', {
                p_organization_id: params.id,
                p_year: currentYear
            })
