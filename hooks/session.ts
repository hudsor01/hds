...
export function useChecksession<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; sessionToken?: string; user_id?: string }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
